...
class RMSNorm(torch.nn.Module):
    ...
    def _norm(self, x):
        ...
        """
        return torch.rsqrt(torch.mean(x*x, keepdim=True)+self.eps) 
...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        '''
        attn = F.softmax(query@key.transpose(-1,-2) / math.sqrt(self.head_dim), dim=-1)
        return self.attn_dropout(attn)@value
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        self_attn = self.attention(norm_input)
        self_attn_output = x + self_attn
        resid_input = self.ffn_norm(self_attn_output)
        feed_forward_output = self.feed_forward(resid_input)
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0):
        ...
        for _ in range(max_new_tokens):
            ...
            if temperature == 0.0:
                ...
                idx_next = F.softmax(logits,dim=1).unsqueeze(1)
            else:
                '''
                ...
                '''
                idx_next = torch.multinomialrgmax(F.softmax(logits/temperature,dim=1),num_samples=1)
